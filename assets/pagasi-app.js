...
function reportePeriodoHtml(periodo, opts){
  ...
  var trIng=r.pagosWin.length?r.pagosWin.slice(0,30).map(function(p){var tp=(p.esInicial||p.tipoOperacion==='inicial_credito')?'Inicial':'Cuota';return '<tr><td class="mut">'+r.fmtFecha(p.fecha)+'</td><td class="b" style="color:#111">'+esc(p.cli||'—')+'</td><td class="mut mono">'+esc(p.cred||'—')+'</td><td><span class="pill '+(tp==='Inicial'?'pill-b':'pill-g')+'">'+tp+'</span></td><td class="r num g">+'+r.fmtUsd(p.monto)+'</td></tr>';}).join(''):empty(5,'Sin pagos en este período');
  var trEg=r.egresosWin.length?r.egresosWin.slice(0,25).map(function(e){return '<tr><td class="mut">'+r.fmtFecha(e.fecha)+'</td><td class="b" style="color:#111">'+esc(e.concepto||e.descripcion||'—')+'</td><td class="mut">'+esc(e.categoria||'Sin categoría')+'</td><td class="r num rd">-'+r.fmtUsd(e.monto)+'</td></tr>';}).join(''):empty(4,'Sin egresos en este período');
  ...
  +'<div class="rp-body">'
    +sec('Resumen financiero','Generado '+genStr,
       ...
       +'</div>', true)
    +sec('Ingresos del período','Pagos confirmados · '+(r.pagosWin.length>30?'30 de '+r.pagosWin.length:String(r.pagosWin.length)),
    ...
    +sec('Egresos del período','Gastos · '+(r.egresosWin.length>25?'25 de '+r.egresosWin.length:String(r.egresosWin.length)),
