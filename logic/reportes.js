...
function descargarAmortPDF(){
  ...
  for(var i=1;i<=totalCuotas;i++){
    ...
    var estado = pd ? 'âœ“ Pagada' : (i===cuotasPag+1 ? 'Próxima' : 'Pendiente');
  ...
  var estadoBadge = c.mora>0
    ? '<span style="background:#fde7ec;color:#c22;padding:3px 10px;border-radius:12px;font-weight:700;font-size:10.5px">â—Â EN MORA · '+c.mora+' día'+(c.mora!==1?'s':'')+'</span>'
    : c.estado==='completado'
      ? '<span style="background:#e7f7ec;color:#0a7a3f;padding:3px 10px;border-radius:12px;font-weight:700;font-size:10.5px">â—Â COMPLETADO</span>'
      : c.estado==='cancelado'
        ? '<span style="background:#eee;color:#666;padding:3px 10px;border-radius:12px;font-weight:700;font-size:10.5px">â—Â CANCELADO</span>'
        : '<span style="background:#eaf4ff;color:#1f5a9e;padding:3px 10px;border-radius:12px;font-weight:700;font-size:10.5px">â—Â '+(c.estado||'activo').toUpperCase()+'</span>';
...
function descargarEstadoPDF(){
  ...
  for(var i=1;i<=totalCuotas;i++){
    ...
    var estadoTxt = pd ? 'âœ“ Pagada' : (esProx ? 'Próxima' : 'Pendiente');
  ...
  if(tipo==='ingresos'){
    ...
    html_content = '<div class="stat-grid">'
      ...
      +S.motos.filter(function(m){return !m.eliminado;}).map(function(m){
        ...
        return '<tr><td>'+m.id+'</td><td>'+m.modelo+'</td><td>'+(m.anio||'—')+'</td><td>'+(m.vin||'—')+'</td><td>'+(m.color||'—')+'</td><td>$'+Number(m.precio||0).toFixed(2)+'</td><td>'+est+'</td><td>'+(m.cliente||'—')+'</td><td>'+(m.gps?'âœ“':'—')+'</td></tr>';
      }).join('')
      +'</tbody></table>';
